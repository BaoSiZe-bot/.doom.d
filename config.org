...
四方向窗口切换
#+begin_src emacs-lisp
(bind-keys ("C-c w h" . windmove-left)
...
(use-package rust-ts-mode
  :mode "\\.rs\\'")
#+end_src
添加 cppcheck 作为 flymake 后端
#+begin_src emacs-lisp
(with-eval-after-load 'eglot
  (add-to-list 'eglot-stay-out-of 'flymake))
(defgroup cppcheck-flymake nil
  "Flymake backend for Cppcheck."
  :group 'flymake)
(defvar-local cppcheck-flymake--process nil)
(defvar-local cppcheck-flymake--generation 0)
(defcustom cppcheck-flymake-enable nil
  "Cppcheck enable option."
  :type 'string
  :group 'cppcheck-flymake)
(defcustom cppcheck-flymake-template
  "{file}:{line}:{column}: {severity}: {message} [{id}]"
  "Template passed to Cppcheck.

The template is intentionally limited to one argument so that the
Cppcheck invocation remains essentially:

  cppcheck FILE

or:

  cppcheck --template=TEMPLATE FILE"
  :type 'string
  :group 'cppcheck-flymake)

(defvar-local cppcheck-flymake--process nil)

(defun cppcheck-flymake--parse (source output)
  "Parse Cppcheck OUTPUT into Flymake diagnostics for SOURCE."
  (let (diags)
    (with-temp-buffer
      (insert output)
      (goto-char (point-min))
      (while (re-search-forward
              "^\\(.+?\\):\\([0-9]+\\):\\([0-9]+\\): \\(error\\|warning\\|style\\|performance\\|portability\\|information\\): \\(.*?\\) \\[\\([^]]+\\)\\]$"
              nil t)
          ...
          (when region
            (push
             (flymake-make-diagnostic
              ...
              (cdr region)
              type
              (format "%s [%s]" message id))
             diags))))
      (nreverse diags))))

(defun cppcheck-flymake-backend (report-fn &rest _more)
  "A Flymake backend for Cppcheck."
  (when (process-live-p cppcheck-flymake--process)
    (delete-process cppcheck-flymake--process))

  (setq cppcheck-flymake--process nil)
  (cl-incf cppcheck-flymake--generation)

  (let ((generation cppcheck-flymake--generation))
    (if (not (buffer-file-name))
        (funcall report-fn nil)

      (let* ((source (current-buffer))
             (file (buffer-file-name))
             (output (generate-new-buffer " *cppcheck-flymake*"))
             (template cppcheck-flymake-template)
             (command
              (append
               (list "cppcheck")
               (when cppcheck-flymake-enable
                 (list (concat
                        "--enable="
                        cppcheck-flymake-enable)))
               (when (and template
                          (not (string-empty-p template)))
                 (list (concat "--template=" template)))
               (list file))))

        (setq cppcheck-flymake--process
              (make-process
               :name "cppcheck-flymake"
               :buffer output
               :stderr output
               :command command
               :noquery t
                 ...
                 (when (memq (process-status process)
                   ...
                   (unwind-protect
                       (when (and
                              ...
                              (eq generation
                                  (buffer-local-value
                                   'cppcheck-flymake--generation
                                   source))
                              (eq process
                                  (buffer-local-value
                                   'cppcheck-flymake--process
                                   source)))
        ...
        cppcheck-flymake--process))))
(defun my/eglot-flymake-setup ()
  (interactive)
  (add-hook 'flymake-diagnostic-functions #'eglot-flymake-backend nil t)
  (add-hook 'flymake-diagnostic-functions #'cppcheck-flymake-backend nil t)
  (flymake-mode 1))
(add-hook 'eglot-managed-mode-hook #'my/eglot-flymake-setup)
#+end_src
针对C 家族的缩进和调试设置
#+begin_src emacs-lisp
...
(bind-keys ("C-c f o" . consult-org-agenda)
           ("C-s" . consult-line)
           ("C-c d g" . flymake-show-buffer-diagnostics))
#+end_src
...
(use-package tab-jump-out
  ...
  :hook ((prog-mode text-mode) . tab-jump-out-mode))
#+end_src
...
#+end_src
设为 markdown preview 默认方式
#+begin_src emacs-lisp
(defun eaf-open-file-in-browser (url)
  "使用 eaf 打开本地文件，并强制重命名 buffer 为文件名"
  (interactive "Url: ")
  (let ((filename (file-name-nondirectory url))
        (full-url (concat "file://" url)))
    (eaf-open-browser full-url)
    (let ((waited 0)
          (created-buffer nil))
      (while (and (not created-buffer) (< waited 30))
        (sit-for 0.05)
        (setq waited (1+ waited))
        (eaf-for-each-eaf-buffer
         (when (and (string= (buffer-local-value 'eaf--buffer-url (get-buffer buffer))
                             full-url)
                    (string= (buffer-local-value 'eaf--buffer-app-name (get-buffer buffer))
                             "browser"))
           (setq created-buffer buffer))))
      (when created-buffer
        (with-current-buffer created-buffer
          (eaf-py-proxy-refresh_page)  ; 只刷新一次
          (rename-buffer filename t)))
      filename)))
(setq markdown-live-preview-window-function #'eaf-open-file-in-browser)
#+end_src
启动终端
#+begin_src emacs-lisp
...
(use-package sort-tab
  ...
  (bind-keys ("C-c b]" . sort-tab-select-next-tab)
             ...
             ;; 数字快捷键：C-c b1 到 C-c b9 切换到第1-9个标签页
             ("C-c b1" . (lambda () (interactive) (sort-tab-select-visible-nth-tab 1)))
             ("C-c b2" . (lambda () (interactive) (sort-tab-select-visible-nth-tab 2)))
             ("C-c b3" . (lambda () (interactive) (sort-tab-select-visible-nth-tab 3)))
             ("C-c b4" . (lambda () (interactive) (sort-tab-select-visible-nth-tab 4)))
             ("C-c b5" . (lambda () (interactive) (sort-tab-select-visible-nth-tab 5)))
             ("C-c b6" . (lambda () (interactive) (sort-tab-select-visible-nth-tab 6)))
             ("C-c b7" . (lambda () (interactive) (sort-tab-select-visible-nth-tab 7)))
             ("C-c b8" . (lambda () (interactive) (sort-tab-select-visible-nth-tab 8)))
             ("C-c b9" . (lambda () (interactive) (sort-tab-select-visible-nth-tab 9)))))
#+end_src
**** packages.el
...
(use-package colorful-mode
  :hook (lisp-mode emacs-lisp-mode javascript-mode typescript-ts-mode css-mode conf-mode lua-mode text-mode))
#+end_src
**** packages.el
...
(with-eval-after-load 'dape
  ;; 1. 获取现有的 lldb-dap 配置 plist
  (let* ((old-config (cdr (assoc 'lldb-dap dape-configs)))
         (new-program '(lambda ()
                         (concat "/tmp/" 
         ...
         ;; 2. 更新 plist 中的属性
         (updated-config (thread-first old-config
                           (plist-put :console "externalTerminal")
                           (plist-put :program new-program))))
    ;; 3. 将更新后的配置写回 dape-configs
    (setf (alist-get 'lldb-dap dape-configs) updated-config)))
#+end_src
**** packages.el
...
#+end_src
*** markdown-previewer
前面用的 markdown-mode 和 eaf-markdown-previewer 都依赖于保存，非常不利于公式书写
感谢 taxodium 提供的实现
**** config.el
#+begin_src emacs-lisp :tangle modules/baosize/markdown-previewer/config.el
(defun impatient-markdown-filter (buffer)
  "Define imp markdown filter with KaTeX support and Highlight.js syntax highlighting.
Wrap BUFFER with HTML, render with marked.js, style with github-markdown-css,
render LaTeX math with KaTeX auto-render, and highlight code with Highlight.js."
...
<html>
  ...
  <head>
    ...
    <style>
      html, body {
        background-color: #0d1117; /* GitHub dark 背景色，避免容器外露白 */
        color-scheme: dark;        /* 顺便告诉浏览器原生控件（滚动条等）也用暗色 */
      }
      .markdown-body {
...
(defun impatient-enable-preview-markdown ()
  "Live Preview markdown."
  (interactive)
  (require 'impatient-mode)
  (imp-visit-buffer)
  (imp-set-user-filter 'impatient-markdown-filter))

(defun impatient-disable-preview-markdown ()
...
#+end_src
**** packages.el
#+begin_src emacs-lisp :tangle modules/baosize/markdown-previewer/packages.el
(package! impatient-mode)
#+end_src

*** copilot-el
Emacs 怎么能没有 copilot 集成呢
**** config.el
#+begin_src emacs-lisp :tangle modules/baosize/copilot-el/config.el
(use-package copilot
...
**** packages.el
#+begin_src emacs-lisp :tangle modules/baosize/copilot-el/packages.el
(package! copilot
